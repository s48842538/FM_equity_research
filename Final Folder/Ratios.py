...
def get_financial_ratios(ticker):
    ...
    # Modified function to handle key variations and missing data
    def safe_get_value(dataframe, possible_keys):
        for key in possible_keys:
            try:
                value = dataframe.loc[key].values[0]
                if pd.notna(value):
                    return value
            except KeyError:
                continue
    ...
    # Debugging: Print available keys in balance sheet
    print("Balance Sheet Keys:", balance_sheet.index.tolist())

    # Extract financial values based on available keys
    current_assets = (safe_get_value(balance_sheet, ['Cash And Cash Equivalents', 'Cash and cash equivalents']) or 0) + \
                     (safe_get_value(balance_sheet, ['Receivables', 'Accounts Receivable']) or 0) + \
                     (safe_get_value(balance_sheet, ['Other Current Assets']) or 0)
    ...
    # Calculate ratios
    if current_assets == 0:  # Handle missing or zero current assets
        current_ratio = None
        quick_ratio = None
    else:
        ...
        quick_ratio = (current_assets - safe_get_value(balance_sheet, ['Cash And Cash Equivalents', 'Cash and cash equivalents'])) / current_liabilities if current_liabilities else None
    ...
    # Liquidity Ratio
    liquidity_ratio = ((safe_get_value(balance_sheet, ['Cash And Cash Equivalents', 'Cash and cash equivalents']) or 0) + 
                       (safe_get_value(balance_sheet, ['Receivables', 'Accounts Receivable']) or 0)) / current_liabilities if current_liabilities else None
    ...
    ratios = {
        ...
        "Cash And Cash Equivalents": [safe_get_value(balance_sheet, ['Cash And Cash Equivalents', 'Cash and cash equivalents']), 
                                      safe_get_value(balance_sheet, ['Cash And Cash Equivalents', 'Cash and cash equivalents']), 
